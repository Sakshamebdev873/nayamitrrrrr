...
const renderMarkdown = (text) => {
  ...
  const parseInline = (line) => {
    line = line.replace(/\*\*(.*?)\*\*/g, (_, match) => `<strong>${match}</strong>`);
    ...
    if (listItems.length > 0) {
      ...
      elements.push(
        <ListTag
          ...
          className={`${isOrdered ? "list-decimal" : "list-disc"} pl-6 space-y-2 mt-4 mb-4 text-gray-800`}
        >
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (/^### /.test(line)) {
      flushList();
      elements.push(
        <h3 key={i} className="text-lg font-medium text-gray-800 mt-4 mb-3 leading-snug">
      ...
      flushList();
      elements.push(
        <h2 key={i} className="text-xl font-semibold text-gray-800 mt-6 mb-4 leading-tight">
          <strong>{line.replace(/^## /, "")}</strong>
        </h2>
      );
    } else if (/^# /.test(line)) {
      flushList();
      elements.push(
        <h1 key={i} className="text-2xl font-bold text-gray-800 mt-8 mb-6 leading-tight">
          <strong>{line.replace(/^# /, "")}</strong>
        </h1>
      );
    } else if (/^[-*] /.test(line)) {
...
{Sidebardata.map((item,index)=>{
   ...
   return <NavLink to={path} key={index} className="flex gap-4 py-3.5 px-6" >
<img src={img} alt="#" className="w-[18px] h-[16px]"/>
<p className="font-normal text-[14px] leading-[14px] text-white " >{text}</p>
   </NavLink>
})}
        </div>
      </div>
      ...
      <div className="flex flex-col justify-between w-full h-screen">
        ...
        <div className="flex justify-between items-center gap-2">
          <div className="h-[77px] w-[82.2vw] bg-[#4338CA] shadow-2xl border-none border-gray-200 flex items-center px-4">
...
>
  ...
  <button
    ...
    className={`px-3 rounded-md border ${recognizing ? 'bg-yellow-100' : 'bg-gray-100'} border-gray-300`}
